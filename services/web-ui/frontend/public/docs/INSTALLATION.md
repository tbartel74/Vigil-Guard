...
The Web UI backend automatically generates a secure admin password on first startup:

1. Start backend: `docker-compose up -d web-ui-backend`
2. **Immediately check console output**: `docker logs vigil-web-ui-backend | grep "Password:"`
3. **Copy the 32-character password** - it will not be shown again
4. Login to http://localhost:5173/ui with `admin/<generated-password>`
...
# Login to Web UI with password from backend logs
