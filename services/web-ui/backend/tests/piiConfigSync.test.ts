...
describe('Redaction Token XSS Protection', () => {
  ...
  describe('Invalid tokens (should reject XSS)', () => {
    const xssVectors = [
      ...
      '<svg/onload=alert(1)>',
      'eval("alert(1)")',
      // Note: 'expression(alert(1))' is NOT an XSS vector in redaction token context
      // It only contains safe characters: letters, digits, parentheses
      // XSS requires HTML/JS special chars: <>&"'=/:;
    ];
