...
async function searchTwoPhase(embedding, topK = 5, deltaThreshold = 0.05) {
    ...
    // Use max similarity for primary metric
    ...
    // Also calculate top-3 average for more robust comparison
    const attackAvgTop3 = attackMatches.length > 0
        ? attackMatches.slice(0, 3).reduce((sum, m) => sum + m.similarity, 0) / Math.min(3, attackMatches.length)
        : 0;
    const safeAvgTop3 = safeMatches.length > 0
        ? safeMatches.slice(0, 3).reduce((sum, m) => sum + m.similarity, 0) / Math.min(3, safeMatches.length)
        : 0;

    // Delta: positive means more similar to attacks
    const delta = attackMaxSim - safeMaxSim;
    const deltaAvg = attackAvgTop3 - safeAvgTop3;
    ...
    // Tier 0a: Very high SAFE match (>0.90) with negative delta - definitely safe
