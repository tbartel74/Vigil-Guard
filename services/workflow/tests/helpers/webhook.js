...
// Webhook authentication - sourced from environment or file
// Token is stored in /config/.webhook-token (generated by install.sh)
// For tests: set WEBHOOK_AUTH_TOKEN env var or mount /config volume
...
/**
 * Get webhook auth token for test requests
 *
 * Token sources (in priority order):
 * 1. WEBHOOK_AUTH_TOKEN environment variable (from .env or CI)
 * 2. /config/.webhook-token file (Docker mounted volume)
 * 3. services/workflow/config/.webhook-token (local dev fallback)
 *
 * NOTE: The public /api/plugin-config endpoint intentionally does NOT
 * return the token for security reasons. Use env var or file instead.
 */
async function getWebhookAuthToken() {
  ...
  // Priority 1: Environment variable (recommended for CI/testing)
  const envToken = process.env.WEBHOOK_AUTH_TOKEN;
  if (envToken) {
    // SECURITY: Validate token format (prevent empty or malformed tokens)
    if (envToken.length < 32) {
      console.warn(
        `⚠️  Security warning: WEBHOOK_AUTH_TOKEN is too short (${envToken.length} chars). ` +
        `Recommended minimum: 32 characters. Token may be weak or invalid.`
      );
    }

    cachedWebhookToken = envToken;
    return cachedWebhookToken;
  }

  // Priority 2: Docker mounted config volume
  const dockerTokenPath = '/config/.webhook-token';
  if (existsSync(dockerTokenPath)) {
    try {
      ...
      cachedWebhookToken = readFileSync(dockerTokenPath, 'utf8').trim();
  ...
  // Priority 3: Local dev fallback (relative to project root)
  const localTokenPath = resolve(__dirname, '../../config/.webhook-token');
  if (existsSync(localTokenPath)) {
    try {
      // SECURITY: Validate file permissions (should be 0600 or stricter)
      const stats = await import('fs').then(m => m.promises.stat(localTokenPath));
      const mode = stats.mode & 0o777; // Extract permission bits

      // Warn if file is readable by group or others
      if (mode & 0o077) {
        console.warn(
          `⚠️  Security warning: ${localTokenPath} has insecure permissions (${mode.toString(8)}). ` +
          `Recommended: 0600 (owner read/write only). Run: chmod 600 ${localTokenPath}`
        );
      }

      cachedWebhookToken = readFileSync(localTokenPath, 'utf8').trim();
      ...
      console.warn('⚠️  Failed to read webhook token from local config:', error.message);
    }
  }

  console.warn(
    '⚠️  Webhook token not found. Set WEBHOOK_AUTH_TOKEN env var or ensure /config/.webhook-token exists.\n' +
    '    Tests may fail authentication if webhook requires token.'
  );
  return '';
}
...
/**
 * Escape string for safe use in ClickHouse SQL string literals
 ...
 * @returns {string} Escaped string safe for ClickHouse SQL
 */
function escapeClickHouseString(str) {
...
export async function waitForClickHouseEvent(criteria = {}, maxWaitMs = 10000) {
  ...
  while (Date.now() - startTime < maxWaitMs) {
    ...
    // SECURITY: Use escapeClickHouseString to prevent SQL injection
    const conditions = [];
    if (criteria.original_input) {
      conditions.push(`original_input = '${escapeClickHouseString(criteria.original_input)}'`);
    }
    if (criteria.sessionId) {
      conditions.push(`sessionId = '${escapeClickHouseString(criteria.sessionId)}'`);
    }
...
export async function verifyClickHouseLog(originalInput) {
  try {
    ...
    const query = `
      ...
      FROM n8n_logs.events_v2
      WHERE original_input = '${escapeClickHouseString(originalInput)}'
      FORMAT JSON
    `;
